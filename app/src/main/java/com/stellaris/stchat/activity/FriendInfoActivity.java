...
import com.stellaris.stchat.utils.NativeImageLoader;
import com.stellaris.stchat.view.FriendInfoView;
...
public class FriendInfoActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (mIsFromContact || mIsFromSearch || mFromGroup || mIsAddFriend) {
            ...
            mGroupId = getIntent().getLongExtra(StApplication.GROUP_ID, 0);
            Conversation conv;
            if (mGroupId == 0) {
    ...
    public void startChatActivity() {
        if (mIsFromContact || mIsAddFriend || mIsFromSearch || mFromGroup) {
            ...
            intent.putExtra(StApplication.CONV_TITLE, title);
            intent.putExtra(StApplication.TARGET_ID, mUserInfo.getUserName());
            intent.putExtra(StApplication.TARGET_APP_KEY, mUserInfo.getAppKey());
            startActivity(intent);
        } else {
            if (mGroupId != 0) {
                Intent intent = new Intent();
                intent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
                intent.putExtra(StApplication.TARGET_ID, mTargetId);
                intent.putExtra(StApplication.TARGET_APP_KEY, mTargetAppKey);
                intent.setClass(this, ChatActivity.class);
                startActivity(intent);
            } else {
                Intent intent = new Intent();
                intent.putExtra("returnChatActivity", true);
                intent.putExtra(StApplication.CONV_TITLE, mTitle);
                setResult(StApplication.RESULT_CODE_FRIEND_INFO, intent);
            }
        }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == StApplication.RESULT_CODE_EDIT_NOTENAME) {
            mTitle = data.getStringExtra(StApplication.NOTENAME);
            FriendEntry friend = FriendEntry.getFriend(StApplication.getUserEntry(), mTargetId, mTargetAppKey);
            if (null != friend) {
