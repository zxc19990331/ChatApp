    ...
    public void onEvent(NotificationClickEvent notificationClickEvent) {
        ...
        if (msg != null) {
            ...
            if (type == ConversationType.single) {
                ...
                notificationIntent.putExtra(StApplication.TARGET_ID, targetId);
                notificationIntent.putExtra(StApplication.TARGET_APP_KEY, appKey);
            } else {
                conv = JMessageClient.getGroupConversation(Long.parseLong(targetId));
                notificationIntent.putExtra(StApplication.GROUP_ID, Long.parseLong(targetId));
            }
            notificationIntent.putExtra(StApplication.CONV_TITLE, conv.getTitle());
